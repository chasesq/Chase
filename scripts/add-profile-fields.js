...
async function addProfileFields() {
  ...
  try {
    console.log('[v0] Adding new profile fields to users table...')
    ...
    for (const migration of migrations) {
      try {
        console.log(`[v0] Adding column: ${migration.name}...`)
        await client.query(migration.sql)
        console.log(`[v0] Successfully added column: ${migration.name}`)
      } catch (err) {
        // Column might already exist, continue
        console.log(`[v0] Column ${migration.name} already exists or error: ${err.message}`)
      }
    }

    // Create indexes for performance
    console.log('[v0] Creating indexes for performance...')
    const indexes = [
      `CREATE INDEX IF NOT EXISTS idx_users_date_of_birth ON users(date_of_birth)`,
      `CREATE INDEX IF NOT EXISTS idx_users_government_id_type ON users(government_id_type)`,
      `CREATE INDEX IF NOT EXISTS idx_users_account_type ON users(account_type_preference)`,
      `CREATE INDEX IF NOT EXISTS idx_users_currency ON users(currency_preference)`,
    ]
    ...
    for (const indexSql of indexes) {
      try {
        ...
        console.log(`[v0] Index creation skipped: ${err.message}`)
      }
    }

    console.log('[v0] Profile fields migration completed successfully!')
  } catch (err) {
